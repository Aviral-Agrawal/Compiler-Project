...
int findArithmeticExpressionType(astNode *root, idfTable *localVar, symbolTable *st, astNode *leaf)
{
  ...
  else if(strcmp(root->keyword,"TK_ID")==0)
  {
    ...
    if(type == 2)
    {
      ...
      if(root->nextSibling != NULL && strcmp(root->nextSibling,"<B>")==0 )
      {
  ...
  {
    int type1 = findArithmeticExpressionType(root->firstChild,st,leaf);
    leaf1=leaf;
  }
  temp = root;
  ...
  if(temp!=NULL)
  {
    ...
    while(temp!=NULL)
    {
      int type2 = findArithmeticExpressionType(temp,st,leaf);
      leaf2=leaf;
      temp=temp->nextSibling;
    }
  }
...
void typeCheckerWithinFunction(astNode *root, idfTable *localVar, symbolTable *st)
{
  ...
  {
    int type1,type2;
    if(strcmp(root->firstChild->firstChild,"<TK_ID>")==0)
    {
    ...
    else if(strcmp(root->firstChild->firstChild,"<TK_NUM>")==0)
      type1=0;
    else if(strcmp(root->firstChild->firstChild,"<TK_NUM>")==1)
      type1=1;

      // for type 2
    if(strcmp(root->firstChild->nextSibling->nextSibling->firstChild,"<TK_ID>")==0)
    {
    ...
    else if(strcmp(root->firstChild->nextSibling->nextSibling->firstChild,"<TK_NUM>")==0)
      type2=0;
    else if(strcmp(root->firstChild->nextSibling->nextSibling->firstChild,"<TK_NUM>")==1)
      type2=1;
  ...
  // recursive calls for children and siblings
  if(root->firstChild!=NULL)
  typeCheckerWithinFunction(root->firstChild,st);
  ...
  if(temp!=NULL)
  {
    ...
    while(temp!=NULL)
    {
      typeCheckerWithinFunction(temp,st);
      temp=temp->nextSibling;
    }
  }
}
...
int semanticAnalyzer(astNode *root)
{
  ...
  populateSymbolTable(astNode *root,symbolTable* st);
  typeChecker(astNode *root, symbolTable *st);
