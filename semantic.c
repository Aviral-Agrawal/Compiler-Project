...
void semanticAnalysis(astNode* root, symbolTable* st, char* functionName)//<mainFunction> not handled
//functionName is used only when root == <function> and then it makes a recusive call
{
    ...
    while(gt!=NULL)
    {
      ...
      while(temp!=NULL)
      {
        if(strcmp(temp->keyword,gt->keyword)==0)
        {
          ...
          if(count>1)
          {
            printf("Line %d : Global Variable %s is being redefined\n", temp->lineNo, temp->keyword);
          }
        }
        temp = temp->next;
      }
  ...
  if(strcmp(root->keyword, "<function>") == 0)
  {
    ...
    if(funOcc != 0)
    {
      printf("Line %d : The function %s is being overloaded.\n", root->firstChild->tk.lineNo, root->firstChild->tk.lexeme);
      return;
    }
      ...
      for(i=0;i<3;i++)
      {
        ...
        while(i1!=NULL)
        {
          ...
          while(t1!=NULL)
          {
            ...
            if(count>1)
              {
                printf("Line %d : Local Variable %s is being redefined\n", t1->lineNo, i1->keyword);
    ...
    for(i=0;i<arrSize;i++)
    {
      if(arr[i]!=1)
      {
        printf("Error in Function: %s Output Parameter : %s Not updated\n",fName,outTab->keyword);
      }
      outTab = outTab->next;
    }
  ...
  else if(strcmp(root->keyword, "<iterativeStmt>") == 0)
  {
      ...
      astNode* temp = root->firstChild->nextSibling;//<stmt>
  ...
  else if(strcmp(root->keyword, "<ioStmt>") == 0)
  {
    ...
    else//<singleOrRecId>==> TK_ID TK_DOT  TK_FIELDID
    {
        ...
        while(fieldItr != NULL)
        {
            if(strcmp(temp->firstChild->nextSibling->tk.lexeme, fieldItr->keyword) == 0)
                return;
        }
        printf("Line %d : The field %s is incorrect for the record.\n", lNo, temp->firstChild->nextSibling->tk.lexeme);
    }
  ...
  else if(strcmp(root->keyword, "<returnStmt>") == 0)
  {
	  ...
	  {
		  int foundInInput = findIdentifier(containingFunction->inputParams, returnList->firstChild->tk.lexeme);
	      int foundInOutput = findIdentifier(containingFunction->outputParams, returnList->firstChild->tk.lexeme);
	      int foundInLocal = findIdentifier(containingFunction->localVariable, returnList->firstChild->tk.lexeme);
	      int foundInGlobal = findIdinGlobal(st, returnList->firstChild->tk.lexeme);
	      if(foundInInput == -1 && foundInOutput == -1 && foundInLocal == -1 && foundInGlobal == -1)
	          printf("Line %d : The variable %s is undeclared.\n", lNo, returnList->firstChild->tk.lexeme);
  ...
  {
      astNode* astItr = root->firstChild;
      while(astItr != NULL)
      {
          if(astItr->leaf != 1)
          	  semanticAnalysis(astItr, st, NULL);
          astItr = astItr->nextSibling;
      }
